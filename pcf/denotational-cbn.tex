...
\item
  \marginpar{За $\texttt{if}$ вижте \Problem{basic-operations:if:continuous}.}
...
\item
  \marginpar{За $\texttt{eval}$ вижте \Problem{eval}.}
  Нека $\tau \equiv \tau_1 \tau_2$. Тогава
...
\begin{proof}
  ...
  \marginpar{Изображението $f \times g$ е дефинирано в \Prop{cartesian-product:pair-continuous}.}
  \begin{itemize}
  \item
  ...
  \item
    ...
    \marginpar{Тук означаваме \[\overline{u}_k = (u_{1,k},\dots,u_{n,k}).\]}
  ...
  \item
    ...
    : \vv{nat}$.
    \marginpar{Използваме, че композиция на непрекъснати изображения е непрекъснато изображение. За произволни $\overline{u} \in \val{\Gamma}$,
    \begin{align*}
  ...
  \item
    ...
    \end{align*}
    \marginpar{За произволни $\overline{u} \in \val{\Gamma}$,
    \begin{align*}
      & (\texttt{eval} \circ (\val{\tau_1}_\Gamma \times \val{\tau_2}_\Gamma))(\overline{u}) =\\
      & \texttt{eval}((\val{\tau_1}_\Gamma \times \val{\tau_2}_\Gamma)(\overline{u})) = \\
      & \texttt{eval}(\val{\tau_1}_\Gamma(\overline{u}), \val{\tau_2}_\Gamma(\overline{u})) \df \\
  ...
  \item
    ...
    \marginpar{Непрекъснатото изображението $Y$ е дефинирано \Th{Y}. За произволни $\overline{u} \in \val{\Gamma}$,
    \begin{align*}
      & (Y \circ \val{\tau'}_\Gamma)(\overline{u}) = \\
      & Y(\val{\tau'}_\Gamma(\overline{u})) = \\
      & \lfp(\val{\tau'}_\Gamma(\overline{u})) \df \\
      & \val{\fix(\tau')}_\Gamma(\ov{u}).
    \end{align*}}
    Следователно
