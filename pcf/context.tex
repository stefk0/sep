...
%   \begin{align*}
%     \val{(\lamb{x}{a}{\mu_i(\tau_i \vv{x})})\rho} & = \val{\mu_i}(\val{\tau_i}(\val{\rho}))\\
%                                                   & = \val{\mu_i(\tau_i \rho)}.
%   \end{align*}
%   Тогава според \Th{pcf:context:connection} е достатъчно да докажем, че
...
%   % Първо, ще използваме, че щом $\tau_1 \eqCtx \tau_2 : \arr{a}{b}$, то за произволен затворен терм $\rho$
%   % от тип $\vv{a}$ е изпъленено, че $\tau_1 \rho \eqCtx \tau_2 \rho : \vv{b}$.

%   % Второ, ще използваме, че щом $\mu_1 \eqCtx \mu_2 : \arr{a}{b}$, то за произволен затворен терм $\rho$
%   % от тип $\vv{a}$ е изпъленено, че $\mu_1 \rho \eqCtx \mu_2 \rho : \vv{b}$.
% \end{hint}
