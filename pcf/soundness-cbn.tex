...
Тогава
\begin{align*}
  \val{\tau} & = \val{\lamb{y}{nat}{\fix(\lamb{x}{nat}{\vv{x}})}}\\
             & = \curry(\underbrace{\val{\fix(\lamb{x}{nat}{\vv{x}})}_\Gamma}_{f}) & \comment \Gamma = \vv{y}:\nat\\
             & = f.
\end{align*}
Да видим сега вътрешната част. От дефиницията на денотационната семантика имаме, че:
\begin{align*}
  \val{\fix(\lamb{x}{nat}{\vv{x}})}_\Gamma(a) & = \lfp(\val{\lamb{x}{nat}{\vv{x}}}_\Gamma(a))\\
                                              & = \lfp(\underbrace{\curry(\val{x}_{\Gamma'})(a)}_{g}) & \comment \Gamma' = \vv{y}:\nat,\vv{x}:\nat.
\end{align*}

Знаем, че $\val{x}_{\Gamma'}(a,b) = b$ за произволни $a,b \in \Nat_\bot$. Тогава за всяко $b \in \Nat_\bot$, $g(b) = \curry(\val{x}_{\Gamma'})(a)(b) = b$, т.е. $g = \texttt{id}$.
Лесно се съобразява, че $\lfp(\texttt{id}) = \bot^{\val{\nat}}$.

Така получаваме, че за всяко $a \in \Nat_\bot$, $f(a) = \lfp(g) = \bot^{\val{\nat}}$, т.е. 
\[\val{\tau} = \bot^{\val{\nat\to\nat}}.\]

От друга страна, обаче, $\tau$ представлява стойност. Следователно, $\tau \Downarrow^0_{\nat\to\nat} \tau$.
