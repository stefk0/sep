...
\marginpar{
  ...
  Също така, няма как да дефинираме релация $\tau \Downarrow_{\vv{P}} L$, където $L$ е език, защото все пак искаме всяко изчисление да трае краен брой стъпки, а това няма как да стане, когато $L$ е безкраен език.
}
...
Щом сме фиксирали програмата $\vv{P}$, с която искаме да работим, това означава, че вече знаем и буквите, които се срещат в програмата,
  ...
  \begin{figure}[H]
    \begin{subfigure}[b]{0.5\textwidth}
      \begin{prooftree}
        ...
        \UnaryInfC{$\varepsilon \opsemReg{0} \varepsilon$}
      \end{prooftree}
    \end{subfigure}
    ~
    \begin{subfigure}[b]{0.5\textwidth}
      \begin{prooftree}
        \AxiomC{$\tau_i \opsemReg{\ell} \beta$}
        \AxiomC{$\alpha = a\cdot \beta$}
        \RightLabel{\scriptsize{(rec)}}
        \BinaryInfC{$a \cdot X_i \opsemReg{\ell+1} \alpha$}
      \end{prooftree}
    \end{subfigure}
...
% Също така, когато се подразбира за коя програма говорим, можем да пишем направо и $\tau \Downarrow \alpha$  и $\tau \Downarrow^\ell \alpha$.
...
\begin{example}
  ...
  \[L = \{a\}^\star \cdot L.\]
  \marginpar{Този пример ни подсказва защо се интересуваме от най-малкото решение на една система, а не просто от някое решение на систамата.}
...
\begin{proof}
  ...
  \item
    ...
    \begin{prooftree}
      \AxiomC{$\tau_j \opsemReg{\ell-1} \beta$}
      \RightLabel{\scriptsize{(rec)}}
      \UnaryInfC{$a \cdot X_j \opsemReg{\ell} a \cdot \beta$}
    \end{prooftree}
  ...
  \item
    ...
    \begin{prooftree}
      \AxiomC{$\rho \opsemReg{\ell-1} \alpha$}
  ...
  \item
    Сега нека $\tau \equiv \rho + \mu$ и последното правило, което сме приложили е следното:
    \begin{prooftree}
      \AxiomC{$\mu \opsemReg{\ell-1} \alpha$}
      \RightLabel{\scriptsize{(right-or)}}
      \UnaryInfC{$\rho + \mu \opsemReg{\ell} \alpha$}
    \end{prooftree}
