...
\marginpar{
  Тук е важно да се отбележи, че дефиницията на операционната семантика не можем да я дадем с индукция по построението на изрази.
  Проблемът е в случая (rec).
  
  Също така, няма как да дефинираме релация $\tau \opsem{}{R} L$, където $L$ е език, защото все пак искаме всяко изчисление да трае краен брой стъпки, а това няма как да стане, когато $L$ е безкраен език.}
  ...
  \begin{figure}[H]
    \begin{subfigure}[b]{0.5\textwidth}
      \begin{prooftree}
        ...
        \UnaryInfC{$\varepsilon \opsem{0}{R} \varepsilon$}
      \end{prooftree}
    \end{subfigure}
    ~
    \begin{subfigure}[b]{0.5\textwidth}
      \begin{prooftree}
        \AxiomC{$\tau_i \opsem{\ell}{R} \beta$}
        \AxiomC{$\alpha = a\cdot \beta$}
        \RightLabel{\scriptsize{(rec)}}
        \BinaryInfC{$a \cdot X_i \opsem{\ell+1}{R} \alpha$}
      \end{prooftree}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.5\textwidth}
      \begin{prooftree}
        ...
        \UnaryInfC{$\tau_1 + \tau_2 \opsemReg{\ell+1} \alpha$}
      \end{prooftree}
    \end{subfigure}
    ~
    \begin{subfigure}[b]{0.5\textwidth}
...
\begin{example}
  \marginpar{На всяка променлива от програмата $\vv{R}$ съответства състояние на автомата.}
...
\index{денотационна семантика}
\label{reg:sect:denotational-semantics}

\marginpar{Тук отново приемаме, че сме работим с предварително фиксирана програма какво в Раздел~\ref{reg:sect:operational-semantics}.}
...
\marginpar{Програмата е просто редове с текст. На този текст съотвества система от уравнения от непрекъснати изображения.}
...
\begin{example}
  ...
  \marginpar{От \Problem{cartesian-product:continuous} знаем, че $\Gamma$ е непрекъснато изображение.}
...
\marginpar{\Example{reg:least-solution} ни подсказва защо се интересуваме от най-малкото решение на една система, а не просто от някое решение на систамата.

\todo Какъв краен автомат съответства на системата от \Example{reg:least-solution}?}
...
Нека оттук нататък да фиксираме една програма $\vv{R}$ на езика \REG:
\begin{SystemEq}
  X_1 & = & \tau_1[X_1,\dots,X_n]\\
  & \vdots & \\
  X_n & = & \tau_n[X_1,\dots,X_n].  
...
\marginpar{Да напомним, че $L_i = \bigcup_k L^k_i$, където:
  \begin{align*}
    & L^0_i = \emptyset\\
...
\begin{proof}
  ...
  \item
    ...
    \begin{figure}[H]
      \begin{subfigure}[b]{0.5\textwidth}
        \begin{prooftree}
          \AxiomC{$\rho \opsem{}{R} \alpha$}
          \LeftLabel{\scriptsize{(left-or)}}
          \UnaryInfC{$\rho + \mu \opsem{}{R} \alpha$}
        \end{prooftree}
        \vspace*{2mm}
      \end{subfigure}
      ~
  ...
  \item
    ...
    От правилата на операционната семантика имаме, че:
    \begin{prooftree}
      \AxiomC{$\tau_j \opsem{}{R} \beta$}
      \RightLabel{\scriptsize{(rec)}}
      \UnaryInfC{$a \cdot X_j \opsem{}{R} a \cdot \beta$}
    \end{prooftree}
...
\begin{proof}
  ...
  \item
    ...
    \begin{prooftree}
      \AxiomC{$\tau_j \opsem{\ell-1}{R} \beta$}
      \RightLabel{\scriptsize{(rec)}}
      \UnaryInfC{$a \cdot X_j \opsem{\ell}{R} a \cdot \beta$}
    \end{prooftree}
  ...
  \item
    ...
    \begin{prooftree}
      \AxiomC{$\rho \opsem{\ell-1}{R} \alpha$}
      \RightLabel{\scriptsize{(left-or)}}
      \UnaryInfC{$\rho + \mu \opsem{\ell}{R} \alpha$}
    \end{prooftree}
