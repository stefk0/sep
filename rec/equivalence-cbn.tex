...
\begin{proposition}\label{pr:rec:op-name-inclusion1}
  ...
  \marginpar{Тук използваме наготово \Problem{cartesian-product:continuous}.}
...
\begin{proof}
  ...
  \item
    ...
    \begin{prooftree}
      ...
      \UnaryInfC{$\mu_1 \opsemFun{\ell_1} \vv{a}_1$}
      \AxiomC{$\vdots$}
      % \RightLabel{\scriptsize{(Извод с дълж. $\ell_2$)}}
      \UnaryInfC{$\mu_2 \opsemFun{\ell_2} \vv{a}_2$}
      \RightLabel{\scriptsize{правило (plus)}}
      \BinaryInfC{$\mu_1 + \mu_2 \opsemFun{\ell_1+\ell_2+1} \vv{a}$}
    \end{prooftree}
  ...
  \item
    ...
    \begin{prooftree}
      ...
      \UnaryInfC{$\tau_i[\vv{x}_1/\mu_1,\dots,\vv{x}_{m_i}/\mu_{m_i}] \opsemFun{\ell-1} \vv{a}$}
      \RightLabel{\scriptsize{правило (cbn)}}
      \UnaryInfC{$\vv{f}_i(\mu_1,\dots,\mu_{m_i}) \opsemFun{\ell} \vv{a}$}
    \end{prooftree}
...
\begin{proposition}
  ...
  Да разгледаме една програма \vv{P} в езика \FUN, като
...
\begin{lemma}
  \marginpar{$\Gamma$ е непрекъснат според \Problem{cartesian-product:continuous}.}
...
\begin{framed}
  \begin{theorem}
    За всяка рекурсивна програма $\vv{P}$ на езика \FUN е изпълнено:
...
\begin{proof}
  ...
  \marginpar{Случаят, когато $\bot$ е измежду $\ov{a}$ е очевиден.}
