...
\begin{remark}
  ...
  \begin{prooftree}
    \AxiomC{$\tau_i[\vv{x}_1/\vv{b}_1,\dots,\vv{x}_{m_i}/\vv{b}_{m_i}] \opsemFun{\ell_0} \vv{a}$}
    \AxiomC{$\mu_1 \opsemFun{\ell_1} \vv{b}_1$}
    \AxiomC{$\cdots$}
    \AxiomC{$\mu_{m_i} \opsemFun{\ell_{m_i}} \vv{b}_{m_i}$}
    \LeftLabel{\scriptsize{($\ell = \sum_{k \leq m_i}\ell_k $)}}
    \RightLabel{\scriptsize{(cbv)}}
    \QuaternaryInfC{$\vv{f}_i(\mu_1,\dots,\mu_{m_i}) \opsemFun{\ell+1} \vv{a}$}
  \end{prooftree}
\end{remark}
