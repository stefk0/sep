\chapter{Области на Скот}
\index{област на Скот}

В тази глава ще разгледаме понятията, които са ни нужни за дефинирането на понятието денотационна семантика на една програма.

\input{domains-basic/partial-order}

\section{Конструкции}

Ще разгледаме няколко конструкции, с които ще видим как можем да строим по-сложни области на Скот.

\input{domains-basic/flat-domain}

\input{domains-basic/product}

\input{domains-basic/general-mappings}

\input{domains-basic/monotone-mappings}

\input{domains-basic/continuous-mappings}

\input{domains-basic/strict-mappings}

\input{domains-basic/strict-restrictions}

\input{domains-basic/lfp}

\input{domains-basic/system}
\newpage
\input{domains-basic/compact}
\newpage
\input{domains-basic/problems}


%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../sep"
%%% End:
