...
\begin{thm}
  ...
  \marginpar{\cite[стр. 178]{theory-computation2017}}
...
\begin{proof}
  ...
  \begin{itemize}
  \item 
    Да фиксираме произволно $m$. Тогава можем да подредим елементите на множеството $\{a_{n,m} \mid n \in \Nat\}$ във възходящ ред относно
     $\sqsubseteq$:
    \[a_{0,m} \sqsubseteq a_{1,m} \sqsubseteq a_{2,m} \sqsubseteq \ldots\]
    \marginpar{По дефиниция, всяка монотонно растяща редица в област на Скот притежава точна горна граница.}
    Следователно тя има точна горна граница $b_m \dff \bigsqcup_n \{a_{n,m} \mid n \in \Nat\}$.
  \item
    ...
    която притежава точна горна граница $c_n \dff \bigsqcup_m \{a_{n,m} \mid m \in \Nat\}$.
  \end{itemize}
...
Да напомним, че релацията \[f \sqsubseteq g \dfff (\forall a\in A)[f(a) \sqsubseteq^\B g(a)]\]
е частична наредба и изображението $\Omega(a) \dff \bot^\B$ за всяко $a\in \A$, е непрекъснато и е най-малкият ѝ елемент
в $\Cont{\A}{\B}$.
\begin{framed}
  \begin{cor}
    $(\Cont{\A}{\B}, \sqsubseteq, \Omega)$ е област на Скот.
  \end{cor}
\end{framed}
...
\begin{proof}
  \marginpar{Обобщете това твърдение за $n > 2$}
  ...
  Нека $\{\pair{a_n,b_n}\}_n$ е монотонно растяща редица в $\A_1\times \A_2$.
  Понеже $\bigsqcup_n\pair{a_n,b_n} = \pair{\bigsqcup_na_n,\bigsqcup_n b_n}$, ще докажем, че 
