...
Тогава $\A \times \B \dff (A \times B,\sqsubseteq,\bot)$, където
\begin{itemize}
\item
...
\begin{itemize}
\item
  Ако $n = 2$, то $\prod^2_{i=1} \A_i \dff \A_1 \times \A_2$.
\item
  Ако $n > 2$, то $\prod^n_{i=1} \A_i \dff (\prod^{n-1}_{i=1}\A_i) \times \A_n$.
\end{itemize}
