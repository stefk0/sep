\section{Езикът REG}\index{език!регулярен}

\subsection{Регулярни изрази}
...
описва термовете на езика $\texttt{REG}$, където $a$ означава буква от азбуката $\Sigma$, а $X$ означава променлива. Нека имаме и безкраен набор от променливи $X_0,X_1,\dots$.
...
Да разгледаме следното обобщение на езика $\texttt{REG}$, който да наречем $\texttt{REG++}$:
\begin{align*}
  \alpha &  ::= \emptyset\ |\ \varepsilon\ |\ a\ |\ \alpha_1 + \alpha_2\ |\ \alpha_1 \cdot \alpha_2\ |\ \alpha^\star_1\\
  \tau & ::= \alpha\ |\ \alpha \cdot X\ |\ \tau_1 + \tau_2.
\end{align*}

\begin{problem}[Правило на Ардън \cite{arden}]
  Да разгледаме следната програма на езика $\texttt{REG++}$:
  \[X = \alpha \cdot X + \beta.\]
  Докажете, че най-малкото решение на системата за тази програма е езикът с регулярен израз $\alpha^\star \cdot \beta$. Докажете, че ако $\varepsilon \not \in \mathcal{L}(\alpha)$, то това решение е и едиствено.
\end{problem}
...
\begin{example}
  ...
  \begin{figure}[H]
    ...
    \caption{Автомат разпознаващ $\{\omega \in \{a,b\}^\star \mid \abs{\omega}_{a} \equiv 0 \bmod\ 3\}$}
  \end{figure}
  На този автомат съответства следната програма $R$, която е програма и на езика $\texttt{REG++}$:
  ...
  Да видим как можем да намерим най-малкото решение на тази система като използваме само правилото на Ардън. Първо разгледаме само последния ред на системата като си мислим, че имаме азбука $\Sigma_3 = \{a,b,X_1,X_2\}$. Тогава получаваме следното:
  \[X_3 = \underbrace{b}_{\alpha} \cdot X_3 + \underbrace{a \cdot X_1}_{\beta}.\]
  Според правилото на Ардън, най-малкото решение на тази система е $b^\star a X_1$.
  ...
  Според правилото на Ардън, най-малкото решение на тази система е $b^\star a b^\star a X_1$.
